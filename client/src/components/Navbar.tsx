    ...
    <nav className="flex items-center justify-between px-6 md:px-16 lg:px-24 xl:px-32 py-4 border-b border-gray-300 bg-white relative transition-all">
      ...
      <div className="hidden sm:flex items-center gap-8">
        ...
        <NavLink to="/cart" className="relative cursor-pointer">
          ...
          <span className="absolute -top-2 -right-3 text-xs text-white bg-green-600 w-[18px] h-[18px] rounded-full flex items-center justify-center">
            3
          </span>
        </NavLink>

        {!user ? (
          <NavLink to="/login" className="cursor-pointer px-8 py-2 bg-green-600 hover:bg-green-700 transition text-white rounded-full">
            Login
          </NavLink>
        ) : (
          <div className="flex items-center space-x-4">
            <span className="text-gray-700">Hi, {user.name}</span>
            <button onClick={logout} className="cursor-pointer px-6 py-2 bg-red-600 hover:bg-red-700 transition text-white rounded-full">
              Logout
            </button>
          </div>
        )}
      ...
      >
        ...
        {!user ? (
          ...
          <button onClick={logout} className="cursor-pointer px-6 py-2 mt-2 bg-red-600 hover:bg-red-700 transition text-white rounded-full text-sm">
            Logout
          </button>
        )}
      </div>
